    ...
    <div className="fixed inset-0 bg-black/70 z-[3000] flex items-center justify-center p-4">
      <Card className="w-full max-w-5xl h-[90vh] flex flex-col bg-white overflow-hidden animate-fade-in shadow-2xl">
        ...
        <div className="flex flex-1 overflow-hidden">
          ...
          {/* Sidebar Tabs */}
          <div className="w-48 bg-slate-50 border-r border-slate-200 flex flex-col p-2 space-y-1 overflow-y-auto">
             <button onClick={() => { setActiveTab('battery'); setShowAddForm(false); }} className={`flex items-center gap-2 p-3 rounded-lg text-sm font-medium transition-colors ${activeTab === 'battery' ? 'bg-white text-blue-600 shadow-sm border border-blue-100' : 'text-slate-600 hover:bg-slate-100'}`}>
                <Battery className="w-4 h-4"/> Baterias
             </button>
             <button onClick={() => { setActiveTab('propeller'); setShowAddForm(false); }} className={`flex items-center gap-2 p-3 rounded-lg text-sm font-medium transition-colors ${activeTab === 'propeller' ? 'bg-white text-blue-600 shadow-sm border border-blue-100' : 'text-slate-600 hover:bg-slate-100'}`}>
                <Fan className="w-4 h-4"/> Hélices
             </button>
             <button onClick={() => { setActiveTab('payload'); setShowAddForm(false); }} className={`flex items-center gap-2 p-3 rounded-lg text-sm font-medium transition-colors ${activeTab === 'payload' ? 'bg-white text-blue-600 shadow-sm border border-blue-100' : 'text-slate-600 hover:bg-slate-100'}`}>
                <Camera className="w-4 h-4"/> Payloads
             </button>
             <button onClick={() => { setActiveTab('accessory'); setShowAddForm(false); }} className={`flex items-center gap-2 p-3 rounded-lg text-sm font-medium transition-colors ${activeTab === 'accessory' ? 'bg-white text-blue-600 shadow-sm border border-blue-100' : 'text-slate-600 hover:bg-slate-100'}`}>
                <Plug className="w-4 h-4"/> Acessórios
             </button>
             <button onClick={() => { setActiveTab('component'); setShowAddForm(false); }} className={`flex items-center gap-2 p-3 rounded-lg text-sm font-medium transition-colors ${activeTab === 'component' ? 'bg-white text-blue-600 shadow-sm border border-blue-100' : 'text-slate-600 hover:bg-slate-100'}`}>
                <Box className="w-4 h-4"/> Peças/Outros
             </button>
          </div>
          ...
          <div className="flex-1 flex flex-col p-4 overflow-hidden bg-white">
             ...
             <div className="flex justify-between items-center mb-4 shrink-0">
                ...
                <Button size="sm" onClick={() => setShowAddForm(true)} className="bg-blue-600 hover:bg-blue-700 text-white shadow-sm">
             ...
             {/* Add Form */}
             {showAddForm && (
                <div className="mb-6 p-4 bg-slate-50 border border-blue-200 rounded-xl animate-fade-in shrink-0 shadow-inner">
                   ...
                   <form onSubmit={handleAddSubmit} className="grid grid-cols-1 md:grid-cols-4 gap-3">
                      ...
                      <div className="md:col-span-1">
                         <Input label="Serial (SN)" placeholder="Opcional para acessórios" value={newItem.serial_number || ''} onChange={e => setNewItem({...newItem, serial_number: e.target.value})} />
                      </div>
                      ...
                      {activeTab === 'battery' && (
                         <>
                           <div className="md:col-span-1"><Input label="Capacidade (mAh)" type="number" required value={newStats.capacity_mah || ''} onChange={e => setNewStats({...newStats, capacity_mah: Number(e.target.value)})} /></div>
                           <div className="md:col-span-1"><Input label="Ciclos Atuais" type="number" required value={newStats.cycles || 0} onChange={e => setNewStats({...newStats, cycles: Number(e.target.value)})} /></div>
                           <div className="md:col-span-1"><Input label="Vida Útil (Max Ciclos)" type="number" required value={newStats.max_cycles || 200} onChange={e => setNewStats({...newStats, max_cycles: Number(e.target.value)})} /></div>
                         </>
                      )}
                      ...
                      {activeTab === 'propeller' && (
                         <>
                           <div className="md:col-span-1"><Input label="Tamanho/Modelo" placeholder='Ex: 21"' value={newStats.size_inch || ''} onChange={e => setNewStats({...newStats, size_inch: e.target.value})} /></div>
                           <div className="md:col-span-1"><Input label="Posição (CW/CCW)" placeholder="Ex: Front Left" value={newStats.position || ''} onChange={e => setNewStats({...newStats, position: e.target.value})} /></div>
                           <div className="md:col-span-1"><Input label="Vida Útil (Max Horas)" type="number" value={newStats.max_hours || 100} onChange={e => setNewStats({...newStats, max_hours: Number(e.target.value)})} /></div>
                         </>
                      )}

                      <div className="md:col-span-4 flex justify-end gap-2 mt-2 pt-2 border-t border-slate-200">
             ...
             <div className="flex-1 overflow-y-auto space-y-3 pr-2 custom-scrollbar">
                {loading ? <div className="text-center p-8 text-slate-400">Carregando estoque...</div> : 
                 ...
                 filteredMaterials.map(mat => (
                   <div key={mat.id} className="border border-slate-200 rounded-lg p-3 hover:shadow-md transition-shadow bg-white flex flex-col md:flex-row gap-4 items-start md:items-center">
                      ...
                      <div className="flex items-center gap-3 w-full md:w-auto md:min-w-[200px]">
                         ...
                         <div>
                            <h4 className="font-bold text-slate-800 text-sm">{mat.name}</h4>
                            <p className="text-xs text-slate-500 font-mono">SN: {mat.serial_number || 'N/A'}</p>
                         </div>
                      </div>
                      ...
                      {/* Actions */}
                      <div className="flex gap-2 justify-end w-full md:w-auto border-t md:border-t-0 border-slate-100 pt-2 md:pt-0">
                         {(mat.type === 'battery' || mat.type === 'propeller') && (
                            <Button size="sm" variant="outline" className="h-8 text-xs bg-slate-50" onClick={() => handleRegisterCycle(mat)}>
