...
export default function Layout({ children }: LayoutProps) {
  ...
  const navigationItems: NavItem[] = [
    ...
    { 
      ...
      icon: DroneIcon,
      adminOnly: false,
      subItems: [{ title: "Cautelas", url: "/cautela", adminOnly: false }]
    },
    { 
  ...
  return (
    <div className="flex h-screen bg-slate-100 overflow-hidden font-sans">
      ...
      <aside className={`fixed inset-y-0 left-0 z-50 w-72 bg-[#7f1d1d] text-white transform transition-transform duration-300 lg:relative lg:translate-x-0 ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full'} shadow-2xl border-r border-red-900/30`}>
        <div className="flex flex-col h-full">
          ...
          <nav className="flex-1 overflow-y-auto px-3 space-y-1 pb-10">
              ...
              return (
                <div key={idx} className="space-y-1">
                  {item.url ? (
                    <Link
                      to={item.url}
                      className={`flex items-center justify-between px-4 py-3 rounded-xl transition-all duration-200 ${isActive ? 'bg-white/15 text-white shadow-inner font-bold border-l-4 border-white' : 'hover:bg-white/5 text-red-100 hover:text-white'}`}
                  ...
                  {item.subItems?.map((sub, sIdx) => {
                    ...
                    return (
                      <Link key={sIdx} to={sub.url || '#'} className={`flex items-center gap-3 px-4 py-2 ml-6 rounded-lg transition-all text-xs ${isSubActive ? 'text-white font-bold bg-white/10' : 'text-red-200 hover:text-white hover:bg-white/5'}`} onClick={() => setIsSidebarOpen(false)}>
          ...
          <div className="p-4 border-t border-red-900/50 bg-black/10">
            ...
            <button onClick={handleLogout} className="flex items-center gap-3 w-full px-4 py-3 rounded-lg text-red-200 hover:bg-red-800 hover:text-white transition-colors text-xs font-bold">
